...
import {
  setAchadoStateViaFuncionarioAction,
  updateNotaViaFuncionarioAction,
} from "./actions";
...
export function AchadoCard({ token, data, mode = "full" }: Props) {
  if (mode === "compact") {
    return <CompactAchadoCard token={token} data={data} />;
  }
  return <FullAchadoCard token={token} data={data} />;
}

function FullAchadoCard({
  token,
  data,
}: {
  token: string;
  ...
  const apply = (state: "resolvido" | "persiste", notaExtra?: string) =>
    new Promise<boolean>((resolve) => {
      start(async () => {
        try {
          const result = await setAchadoStateViaFuncionarioAction(
            token,
            data.achadoId,
            state,
            notaExtra,
  ...
  const handleSaveNota = () => {
    ...
    start(async () => {
      try {
        const result = await updateNotaViaFuncionarioAction(
          token,
          data.achadoId,
          nota,
        );
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button
          ...
          disabled={pending}
          title="Falar com a engenharia sobre este achado"
        >
      ...
      {tipoAtual === "persiste" && editMode === "persiste-edit" ? (
        <div className="space-y-2">
          <Textarea
            value={nota}
            onChange={(e) => setNota(e.target.value)}
            rows={3}
            placeholder="O que aconteceu? (ex: aguardando peça, retorno agendado para 30/05...)"
            disabled={pending}
          />
          {nota !== notaSaved ? (
            <Button
    ...
    >
      ...
      >
        <Badge
          variant="outline"
          className={cn(
            "shrink-0 font-mono text-[10px] bg-transparent",
            CATEGORIA_BADGE_OUTLINE_CLASS[data.categoria],
          )}
        >
          {CATEGORIA_LABELS[data.categoria]}
        </Badge>
        <PrioBadge prioridade={data.prioridade} />
        <div className="min-w-0 flex-1">
          <p className="line-clamp-1 text-sm">
            {data.local ? (
              <span className="font-medium">{data.local} · </span>
            ) : null}
            <span className="text-foreground/80">{data.descricao}</span>
          </p>
        </div>
